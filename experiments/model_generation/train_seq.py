"""
Training Script — Sequence-Only Ablation (No Structure Branch)
===============================================================

Trains ESMSequenceOnlyModel from model_seq.py. This is an ablation
experiment to isolate the contribution of ESM sequence representations
vs the graph/structure encoder.
...
Compare with:
    ...
    - train3.py (--model_type esm3)  → ESM + graph transformer + ESM node features
"""
...
# ============================================================
# Sequence-Only Lightning Module
# ============================================================
class SeqOnlyBindingLitModule(_BaseLitModule):
    """Lightning wrapper for ESMSequenceOnlyModel (no structure branch)."""

    def __init__(self, esm_checkpoint="facebook/esm2_t12_35M_UR50D", freeze_esm=True,
                 n_tune_layers=0, d_model=256, n_cross_heads=8,
    ...
    def _shared_step(self, batch):
        ...
        # Ignore struct_graph (bg) — sequence-only model
        out = self.model(tcr_ids=tcr_esm, mhc_ids=pmhc_esm,
                         tcr_mask=tcr_mask, mhc_mask=pmhc_mask,
...
def train_fold(args):
    ...
    print(f"\n{'='*60}\n  Fold {fold_idx} | {args.model_type.upper()} (Sequence-Only)")
    ...
    if args.phase2_unfreeze_layers > 0 and args.phase2_epochs > 0:
        ...
        model = SeqOnlyBindingLitModule.load_from_checkpoint(best, esm_checkpoint=args.esm_checkpoint)
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Train sequence-only ablation model (no structure encoder)",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    p.add_argument("--model_type", default="seq", choices=["seq"],
                   help="Model type (seq = ESM sequence-only, no graph)")
