"""
Multimodal TCR–pMHC Binding Prediction Model
==============================================

Three modalities:
  1. TCR sequence  →  GRU VAE encoder  →  z_tcr [B, Z]
  2. MHC sequence  →  GRU VAE encoder  →  z_mhc [B, Z]
...
Dependencies:
  ...
  - dgl + dgl.nn.EGNNConv
  - Your existing `layers.py` with SelfAttention / MultiHeadAttention
"""
...
class MultimodalBindingModel(nn.Module):
    ...
    def __init__(
        ...
        # ===== Sequence branch =====
        # TCR encoder / decoder
        self.tcr_enc = GRUEncoder(vocab_size, pad_id, d_model, n_enc_layers, dropout)
        self.tcr_to_mu = nn.Linear(d_model, latent_dim)
        self.tcr_to_logvar = nn.Linear(d_model, latent_dim)
        self.tcr_dec = nn.GRU(d_model, d_model, n_enc_layers, batch_first=True,
        ...
        # MHC encoder / decoder
        self.mhc_enc = GRUEncoder(vocab_size, pad_id, d_model, n_enc_layers, dropout)
        self.mhc_to_mu = nn.Linear(d_model, latent_dim)
        self.mhc_to_logvar = nn.Linear(d_model, latent_dim)
        self.mhc_dec = nn.GRU(d_model, d_model, n_enc_layers, batch_first=True,
                              dropout=dropout if n_enc_layers > 1 else 0.0)
        ...
        self.classifier = nn.Sequential(
            nn.Linear(d_fused, clf_hidden),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(clf_hidden, clf_hidden),
            nn.ReLU(),
            nn.Dropout(dropout),
...
if __name__ == "__main__":
    ...
    model = MultimodalBindingModel(
        ...
        d_model=128, latent_dim=32,
        n_enc_layers=2, n_cross_heads=4,
