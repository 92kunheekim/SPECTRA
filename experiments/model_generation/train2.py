...
class TCRpMHCDataModule(L.LightningDataModule):
    def __init__(self, csv_path, pdb_dir, fold_info, model_type="vae",
        ...
        if model_type in ("esm", "esm2", "esm2_bn", "esm3", "seq", "struct"):
...
class _BaseLitModule(L.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        out, probs, labels = self._shared_step(batch)
        B = len(labels)
        self.log("val/loss", out["loss"], batch_size=B, prog_bar=True, on_step=False, on_epoch=True, sync_dist=True)
        li = labels.long()
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.log("test/loss", out["loss"], batch_size=B, on_step=False, on_epoch=True, sync_dist=True)
...
class ESMBindingLitModule(_BaseLitModule):
    def __init__(self, esm_checkpoint="facebook/esm2_t12_35M_UR50D", freeze_esm=True,
                 ...
                 pos_weight=1.0, lr=3e-4, esm_lr=1e-5, weight_decay=1e-4,
                 warmup_steps=500, max_steps=50000, grad_clip=1.0, pad_id=PAD_IDX):
        super().__init__()
        self.save_hyperparameters(); self._init_metrics()
        ...
        self.model = ESMMultimodalBindingModel(
            ...
            n_tune_layers=n_tune_layers, d_model=d_model, n_cross_heads=n_cross_heads,
    ...
    def configure_optimizers(self):
        esm_mods = (self.model.esm_encoder.esm_model,)
        esm_p, other_p = [], []
        for _, p in self.model.named_parameters():
            if not p.requires_grad: continue
            (esm_p if any(p is ep for m in esm_mods for ep in m.parameters()) else other_p).append(p)
        groups = [{"params": other_p, "lr": self.hparams.lr}]
        if esm_p: groups.append({"params": esm_p, "lr": self.hparams.esm_lr, "weight_decay": 0.01})
        return self._build_optimizer(groups)

# ============================================================
# 4b. Model B2: ESM + Pseudo-Heterogeneous Graph Transformer
# ============================================================
class ESM2BindingLitModule(_BaseLitModule):
    """Lightning wrapper for ESMMultimodalBindingModel2 (hetero graph transformer)."""

    def __init__(self, esm_checkpoint="facebook/esm2_t12_35M_UR50D", freeze_esm=True,
                 n_tune_layers=0, d_model=256, n_cross_heads=8, node_feat_size=20,
                 edge_feat_size=7, struct_hidden_dim=320, struct_edge_hidden=32,
        ...
        self.save_hyperparameters(); self._init_metrics()
        from transformers import EsmForMaskedLM
        from model_llm2 import ESMMultimodalBindingModel2
        esm = EsmForMaskedLM.from_pretrained(esm_checkpoint, local_files_only=os.path.isdir(esm_checkpoint))
        self.model = ESMMultimodalBindingModel2(
...
def train_fold(args):
    ...
    if args.model_type == "vae":
    ...
    elif args.model_type in ("esm2", "esm2_bn"):
        # esm2_bn forces batchnorm; esm2 uses --norm_type (default layernorm)
        norm = "batchnorm" if args.model_type == "esm2_bn" else args.norm_type
        model = ESM2BindingLitModule(
        ...
        model = ESMBindingLitModule(
            ...
            lr=args.lr, esm_lr=args.esm_lr, weight_decay=args.weight_decay,
            warmup_steps=args.warmup_steps, max_steps=ms, grad_clip=args.grad_clip)
    ...
    # Phase 2 (ESM only): unfreeze ESM layers for joint fine-tuning
    # All learning rates are scaled down to avoid catastrophic forgetting.
    # The optimizer state from Phase 1 is lost (new Trainer creates fresh
    # AdamW), so we use a gentler LR and longer warmup to compensate.
    if args.model_type in ("esm", "esm2", "esm2_bn") and args.phase2_unfreeze_layers > 0 and args.phase2_epochs > 0:
        print(f"\n{'='*60}\n  Phase 2: unfreeze {args.phase2_unfreeze_layers} ESM layers, {args.phase2_epochs} epochs\n{'='*60}\n")
        LitClass = ESM2BindingLitModule if args.model_type in ("esm2", "esm2_bn") else ESMBindingLitModule
        model = LitClass.load_from_checkpoint(best, esm_checkpoint=args.esm_checkpoint)
        model.model.set_esm_tuning(freeze=True, n_tune_layers=args.phase2_unfreeze_layers)

        # Scale down ALL learning rates for Phase 2:
        #   - non-ESM params (proj heads, classifier, cross-attn): phase2_lr (default 5e-5)
        #   - ESM backbone params: phase2_esm_lr (default 2e-6, 5x lower than Phase 1)
        #   - LoRA adapters: phase2_lr * 0.5 (already well-tuned, gentle updates)
    ...
    print(f"\nTesting: {best}")
    # Create a fresh trainer for testing to avoid state issues from training
...
def parse_args():
    ...
    p.add_argument("--model_type", choices=["vae","esm","esm2","esm2_bn"], required=True)
    ...
    p.add_argument("--norm_type", choices=["layernorm", "batchnorm"], default="layernorm",
                   help="Normalization in classifier head: layernorm (default) or batchnorm")
    # VAE-specific
    ...
    e.add_argument("--phase2_esm_lr", type=float, default=2e-6,
                   help="ESM backbone LR in Phase 2 (should be much lower than Phase 1 esm_lr)")
    e.add_argument("--phase2_warmup_steps", type=int, default=1000,
                   help="Warmup steps for Phase 2 (longer to compensate for lost optimizer state)")
    # ESM2 hetero structure encoder
    h = p.add_argument_group("ESM2 hetero struct")
    h.add_argument("--struct_hidden_dim", type=int, default=320, help="Hidden dim for hetero graph transformer")
    h.add_argument("--struct_edge_hidden", type=int, default=32, help="Edge feature hidden dim after MLP")
    h.add_argument("--struct_n_layers", type=int, default=3, help="Number of pseudo-hetero transformer layers")
    h.add_argument("--struct_n_heads", type=int, default=4, help="Attention heads per edge-type conv")
    h.add_argument("--struct_out_dim", type=int, default=128, help="Structure encoder output dim")
    # LoRA-specific
